document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('[data-testid="button-play-game"]').forEach(function(btn) {
        ...
        btn.addEventListener('click', function() {
            ...
            if (ctn) {
                ...
                ifr.sandbox = 'allow-scripts allow-same-origin allow-forms';
                ifr.style.width = '100%';
    ...
    if (toggle) {
        toggle.addEventListener('click', function() {
            ...
            menu.className = 'md:hidden py-4 space-y-2';
            var b = document.querySelector('[data-testid="link-browse"]');
            var c = document.querySelector('[data-testid="link-categories"]');

            function linkClone(a, testid) { if (!a) return null; var x = document.createElement('a');
                x.href = a.getAttribute('href');
                x.textContent = a.textContent || '';
                x.className = 'block px-3 py-2 rounded-md';
                x.setAttribute('data-testid', testid); return x; }
            var lb = linkClone(b, 'link-browse-mobile');
            var lc = linkClone(c, 'link-categories-mobile');
            if (lb) menu.appendChild(lb);
            if (lc) menu.appendChild(lc);
